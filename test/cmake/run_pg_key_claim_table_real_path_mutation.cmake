# Production mutation proof for KeyClaimTable. The changed TableGen shape must
# reach only the temporal schema (the last declaration, after committed_group)
# while every other selected byte and table position remains exact, and must
# never appear on the non-temporal or evidence-only paths.
...
foreach(_required COMMITTED MUTATED OUT_ROOT TEMPORAL_INPUT TEMPORAL_SCENARIO
  ...
  if(NOT DEFINED ${_required})
    message(FATAL_ERROR "key-claim-table real-path mutation: missing ${_required}")
  endif()
endforeach()

set(_committed_token "bound_at        timestamptz NOT NULL DEFAULT now()")
set(_mutated_token "bound_at        timestamp   NOT NULL DEFAULT now()")
set(_quorum_marker "CREATE TABLE IF NOT EXISTS quorum_acceptance (")
set(_committed_group_marker "CREATE TABLE IF NOT EXISTS committed_group (")
set(_table_marker "CREATE TABLE IF NOT EXISTS key_claim (")
...
function(_run_schema _binary _input _scenario _out_dir _result_var)
  ...
  if(NOT _rc EQUAL 0)
    message(FATAL_ERROR
      "key-claim-table mutation: neutrino-gen failed (${_rc}):\n${_stdout}\n${_stderr}")
  endif()
...
function(_prove_temporal_path)
  ...
  if(_committed_at EQUAL -1 OR _mutated_at EQUAL -1 OR NOT _stale_at EQUAL -1)
    message(FATAL_ERROR
      "key-claim-table mutation: KeyClaimTable did not change through the "
      "production temporal schema path")
  endif()

  string(FIND "${_committed_schema}" "${_quorum_marker}" _quorum_at)
  string(FIND "${_committed_schema}" "${_committed_group_marker}" _cg_at)
  string(FIND "${_committed_schema}" "${_table_marker}" _table_at)
  if(_quorum_at EQUAL -1 OR _cg_at EQUAL -1 OR _table_at EQUAL -1 OR
     NOT _quorum_at LESS _cg_at OR NOT _cg_at LESS _table_at)
    message(FATAL_ERROR
      "key-claim-table mutation: temporal schema declaration ordering drifted")
  endif()
  ...
  if(NOT "${_normalized_mutated}" STREQUAL "${_committed_schema}")
    message(FATAL_ERROR
      "key-claim-table mutation: schema bytes beyond the selected TableGen "
      "mutation changed")
  endif()
endfunction()
...
function(_prove_excluded_path _name _input _scenario)
  ...
  if(NOT _committed_at EQUAL -1 OR NOT _mutated_at EQUAL -1)
    message(FATAL_ERROR
      "key-claim-table mutation: ${_name} selection emitted the temporal table")
  endif()
  if(NOT "${_committed_schema}" STREQUAL "${_mutated_schema}")
    message(FATAL_ERROR
      "key-claim-table mutation: an unselected node changed ${_name} SQL")
  endif()
endfunction()
...
message(STATUS
  "KeyClaimTable mutation reaches only the temporal production path; all other "
  "SQL bytes and declaration ordering remain exact")
