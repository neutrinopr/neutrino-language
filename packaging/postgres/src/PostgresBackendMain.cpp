// /: the PostgreSQL backend package's external protocol entry.
//
// The PostgreSQL analogue of packaging/solidity/src/SolidityBackendMain.cpp.
// It speaks the versioned subprocess protocol over stdio and renders the
// package-owned SQL artifact set from the VERIFIED BackendProjectionGraph --
// never from a ProcedureView, scenario, or any core-side plan representation.
// That is the whole point of the  boundary: bytes over a protocol, no C++
// ABI edge and no compiler-plan reach-back.
...
std::string emissionReceipt(const std::vector<PostgresSourceFile> &artifacts) {
  std::string material;
  for (const PostgresSourceFile &artifact : artifacts) {
...
// The artifact set is derived ENTIRELY from the verified graph, via the ONE
// befriended sink. This entry deliberately holds no RawSql extraction
// authority: it receives bytes that materializePostgresPackageArtifacts has
// already extracted.
std::vector<PostgresSourceFile>
materializePostgresSources(const projection::BackendProjectionGraph &graph) {
  std::vector<PostgresSourceFile> sources;
  for (std::pair<std::string, std::string> &artifact :
       materializePostgresPackageArtifacts(graph))
    sources.push_back({std::move(artifact.first), std::move(artifact.second)});
  return sources;
}
...
int main(int argc, char **argv) {
  ...
  std::vector<PostgresSourceFile> sources;
  try {
    sources = materializePostgresSources(accepted->graph);
  ...
  for (const PostgresSourceFile &source : sources) {
